...
export default function SearchField({
  ...
  useEffect(() => {
    ...
    for (let [key, value] of Object.entries(idToGroup)) {
      ...
      if (value.name !== "")
        items.push({ key: key, value: value.name, type: "group" });
    }
    for (let [key, value] of Object.entries(idToContact)) {
      if (value.name !== "")
        items.push({ key: key, value: value.name, type: "contact" });
    }
    setItems(items);
  ...
  function changeHandler() {
    ...
    if (ref.current !== undefined) setCurValue(ref.current.value);
  }
  ...
  return (
    <>
      <InputGroup>
        <Form.Control
          ref={ref}
          placeholder="No chat selected, showing all chats."
          aria-label="No chat selected, showing all chats."
